...
namespace CyberSecurityChatbot.Logic
{
    public class KeywordResponder
    {
        ...
        // Returns a response for the first matching keyword found in input, or null if none found
        public string? GetResponse(string input)
        {
            foreach (var kvp in _keywordResponses)
            {
                ...
                {
                    // Return first response from array (or you can randomize later)
                    return kvp.Value[0];
                }
            }
        ...
        // Optionally, get all keywords (for e.g. showing topics)
